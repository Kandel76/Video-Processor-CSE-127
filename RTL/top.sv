...
module top #(
    ...
    input  logic                              clk,
    input  logic                              rst_n,
    input  logic                              frame_start, //cocotb drives this 
    ...
    // Memory write port -- wready_i driven by cocotb, 
    output logic [15:0]                       waddr_o,
    output logic [7:0]                        wdata_o,
    input  logic                              wready_i,

    // Status outputs --  Not Necessary, could be useful for debugging/other puurposes
    ...
    //get parallel adcs for each columns
    genvar gi;
    generate
        for (gi = 0; gi <= COLS; gi++) begin : g_col
            sar_adc #() u_adc (
                .clk          (clk),
                .cmp_o        (cmp_o[gi]), //each adc gets one bit from the comparator array/bank
                .read_en      (adc_read_en),
