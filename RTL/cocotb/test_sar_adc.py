...
#Function to wait for a rising edge to ensure we read the correct value
async def clk_rising_ro(dut):
    await RisingEdge(dut.clk)
    await ReadOnly()
    await NextTimeStep()

#returns state machine names for readility 
def state_name(v):
    return {
        IDLE: "IDLE",
        SAMPLE: "SAMPLE",
...
#ensures we are in the correct state
...
async def reset_dut(dut):
    ...
    await NextTimeStep()
#tests we enter the proper states upon read_en and valid_voltage, with comp_done
...
#goes through the entire sar_adc process, ensuring correct state transitions 
async def drive_to_send(dut):
    ...
    assert_state(dut, SEND, "Expected SEND at last step:")
...
@cocotb.test()
#test checks the adc final value based on x amount of steps 
...
async def test_sar_adc_fsm_transitions(dut):
    ...
    # SAMPLE -> SAMPLE while valid high (read_en stays high with valid in practice)
    await clk_rising_ro(dut)
...
async def test_sar_adc_send_state_sticky(dut):
    ...
    # adc_done is registered the cycle after entering SEND (same always_ff pattern).
    await clk_rising_ro(dut)
