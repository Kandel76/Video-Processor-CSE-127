...
module scanner_to_mem #(
    ...
    input  logic [PIXEL_BUS_WIDTH-1:0] pixel_data,
    input  logic [$clog2(ROWS)-1:0]    pixel_row,
    ...
    logic [PIXEL_BUS_WIDTH-1:0] pixel_data_q;
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (state_q == IDLE && pixel_valid) begin
                pixel_data_q <= pixel_data;
                row_q        <= pixel_row;
            end
        end
    end
    ...
    // byte i = pixels [2*i+1:2*i]  =>  pixel_data[8*i+7 : 8*i]
    assign waddr_i = (row_q * BYTES_PER_ROW) + byte_idx_q;
    assign wdata_i = pixel_data_q[byte_idx_q * 8 +: 8];
